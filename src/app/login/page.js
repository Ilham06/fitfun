import { signInWithGoogle } from "@/lib/auth-actions";
import { ScanLine } from "lucide-react";

export const metadata = {
  title: "Login | FitScan",
};

export default function LoginPage() {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#F5F5F5] px-6">
      <div className="w-full max-w-sm flex flex-col items-center">
        <h1 className="font-display text-4xl font-black text-accent tracking-tight mb-3">
          FitScan
        </h1>
        <p className="text-muted text-center text-sm leading-relaxed mb-12">
          Scan your food. Track your macros.
          <br />
          Transform your body with AI.
        </p>

        <div className="w-48 h-48 rounded-full bg-[#E8F5F0] flex items-center justify-center mb-12">
          <ScanLine size={64} className="text-[#2D9C7E]" />
        </div>

        <form action={signInWithGoogle}>
          <button
            type="submit"
            className="w-full flex items-center justify-center gap-3 h-13 px-6 rounded-xl bg-white border border-border2 shadow-sm text-text font-semibold text-sm hover:border-[#2D9C7E]/30 hover:shadow-md transition-all cursor-pointer"
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24">
              <path
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                fill="#4285F4"
              />
              <path
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                fill="#34A853"
              />
              <path
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                fill="#FBBC05"
              />
              <path
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                fill="#EA4335"
              />
            </svg>
            Continue with Google
          </button>
        </form>

        <p className="mt-6 text-xs text-muted2 text-center">
          By continuing, you agree to our{" "}
          <span className="text-muted underline cursor-pointer">
            Privacy Policy
          </span>
        </p>
      </div>
    </div>
  );
}
